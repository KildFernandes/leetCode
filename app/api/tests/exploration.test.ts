...
describe("Prova de Presença", () => {
  const recovered = {
    promptId: "one",
    answer: "uma resposta própria",
    revealedAt: new Date(),
    assessment: "RECOVERED" as const,
  };
  it("só conclui após todos os marcos serem recuperados e haver nota de campo", () => {
    expect(
      canCompleteExploration(
        ["one", "two"],
        [recovered],
        "mudou minha hipótese",
      ),
    ).toBe(false);
    expect(canCompleteExploration(["one"], [recovered], "")).toBe(false);
    expect(
      canCompleteExploration(["one"], [recovered], "mudou minha hipótese"),
    ).toBe(true);
  });
