...
export async function importSolutions(prisma: PrismaClient, root: string) {
  ...
  const files = entries.filter(
    (e) => e.isFile() && e.name.endsWith(".h") && !e.name.includes("-attempt"),
  );
  let imported = 0;
  for (const file of files) {
    const fullPath = path.join(file.parentPath, file.name);
    const relative = path
      .relative(path.dirname(absolute), fullPath)
      .replaceAll(path.sep, "/");
    ...
    const name = match[2]
      ...
      .join(" ");
    const segment = Object.keys(directoryCompetencies).find((key) =>
      fullPath.split(path.sep).includes(key),
    );
    ...
    for (const competencyId of ids)
      await prisma.problemCompetency.upsert({
        where: { problemId_competencyId: { problemId: id, competencyId } },
        update: {},
  ...
  return {
    imported,
    ignoredDrafts: entries.filter(
      (e) => e.isFile() && e.name.includes("-attempt"),
    ).length,
  };
}
