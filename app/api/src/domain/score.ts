export type ScoreEvidence = {
  ...
  latestFailure?: 'breadth'|'retrieval'|'independence'|'transfer'|'explanation';
};
...
export function calculateScore(e: ScoreEvidence) {
  ...
  if (e.latestFailure) {
    const penalty = { breadth: 4, retrieval: 10, independence: 5, transfer: 15, explanation: 2 }[e.latestFailure];
    if (e.latestFailure === 'breadth') breadth = Math.max(0, breadth - penalty);
    if (e.latestFailure === 'retrieval') retrieval = Math.max(0, retrieval - penalty);
    if (e.latestFailure === 'independence') independence = Math.max(0, independence - penalty);
    if (e.latestFailure === 'transfer') transfer = Math.max(0, transfer - penalty);
    if (e.latestFailure === 'explanation') explanation = Math.max(0, explanation - penalty);
  }
  return { score: Math.max(0, Math.min(100, breadth + retrieval + independence + transfer + explanation)), breadth, retrieval, independence, transfer, explanation };
}
